...
// Sélection en cours sur la page de garde, avant confirmation par le bouton
let pendingTeamId = currentTeamId || null;
const preselectedFromUrl = Boolean(getParam("team"));
...
function renderCover() {
  ...
  // --- Sélecteur d'équipe ---
  if (preselectedFromUrl) {
    // Lien direct type ?team=2 : équipe déjà déterminée, on l'affiche simplement
    const team = C.teams.find((t) => t.id === pendingTeamId);
    wrap.appendChild(
      el("div", "team-picked-badge", "", `${checkIconSmall()} Vous êtes dans l'équipe : <strong>${team ? team.name : "?"}</strong>`)
    );
  } else {
    wrap.appendChild(el("div", "eyebrow team-picker-label", "Choisis ton équipe pour commencer"));
    const grid = el("div", "team-grid");
    C.teams.forEach((t) => {
      const btn = el("button", "team-btn", t.name);
      if (pendingTeamId === t.id) btn.classList.add("selected");
      btn.onclick = () => {
        pendingTeamId = t.id;
        render();
      };
  ...
  btn.onclick = () => {
    ...
    localStorage.setItem("roadTo26_currentTeam", currentTeamId);
    view = "go";
    render();
  };
...
function renderFragments(team, state, loc) {
  ...
  const stepMsg = C.stepArrivalMessages && C.stepArrivalMessages[state.stepIndex];
  if (stepMsg) {
    wrap.appendChild(el("div", "step-banner", stepMsg));
  }
...
function renderFinal(team, state) {
  ...
  btn.onclick = () => {
    ...
    if (!allMatch) {
      ...
      const logoHTML = d.logo ? `<img src="${d.logo}" alt="${d.name}" class="final-logo" />` : "";
